...
// For Coupon Code Application
if(isset($_POST['applycoupon'])) {
    $coupon_code = $_POST['coupon_code'];
    $uid = $_SESSION['id'];
    $check_coupon = mysqli_query($con, "SELECT * FROM coupons WHERE code='$coupon_code' AND status='active'");
    if(mysqli_num_rows($check_coupon) > 0) {
        $coupon = mysqli_fetch_array($check_coupon);
...
if(isset($_POST['proceedpayment'])){
 ...
 if(isset($_SESSION['discount'])) {
    $discount_amount = ($gtotal * $_SESSION['discount']) / 100;
    $gtotal -= $discount_amount;
    unset($_SESSION['discount']); // Remove discount after applying
 }
...
<form action="stripe-payment.php" method="POST">
    <input type="hidden" name="totalmount" value="<?php echo $totalAmount; ?>">
